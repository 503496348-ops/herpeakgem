...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "HerPeakGem Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "HerPeakGem Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "HerPeakGem";
  ...
  else if (mode === "responded") modeLabel = t("HerPeakGem responded.");
 ...
 * reasoning trace once HerPeakGem stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("HerPeakGem Exploring… · 8s", settling to "HerPeakGem responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while HerPeakGem is still reasoning /
