# ============================================
# HerPeakGem Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for HerPeakGem
# containing both the FastAPI backend and Next.js frontend
#
# Build/run:
#   docker build -t herpeakgem:local .
#   docker run -p 127.0.0.1:3782:3782 -p 127.0.0.1:8001:8001 \
#     -v herpeakgem-data:/app/data herpeakgem:local
#
# Prerequisites:
...
COPY herpeakgem/__version__.py /app/herpeakgem/__version__.py
...
# Labels
LABEL maintainer="HerPeakGem Team" \
      description="HerPeakGem: AI-Powered Personalized Learning Assistant"
...
COPY herpeakgem/ ./herpeakgem/
COPY herpeakgem_cli/ ./herpeakgem_cli/
...
RUN cat > /etc/supervisor/conf.d/herpeakgem.conf <<'EOF'
[supervisord]
nodaemon=true
logfile=/dev/null
logfile_maxbytes=0
...
RUN sed -i 's/\r$//' /etc/supervisor/conf.d/herpeakgem.conf
...
exec python -m uvicorn herpeakgem.api.main:app --host 0.0.0.0 --port ${BACKEND_PORT}
EOF
...
eval "$(python - <<'PY'
import shlex
from herpeakgem.services.config import export_runtime_settings_to_env
...
# Start supervisord
exec /usr/bin/supervisord -c /etc/supervisor/conf.d/herpeakgem.conf
EOF
...
[program:backend]
command=python -m uvicorn herpeakgem.api.main:app --host 0.0.0.0 --port %(ENV_BACKEND_PORT)s --reload
directory=/app
autostart=true
autorestart=true
stdout_logfile=/dev/fd/1
