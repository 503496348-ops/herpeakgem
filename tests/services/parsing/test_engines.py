...
from herpeakgem.services.parsing.engines import factory
from herpeakgem.services.parsing.types import ParserError
...
def test_text_only_parser_extracts_docx_text(tmp_path) -> None:
    ...
    with zipfile.ZipFile(docx, "w") as zf:
        zf.writestr(
            ...
            <w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
              <w:body>
                <w:p><w:r><w:t>Hello HerPeakGem</w:t></w:r></w:p>
              </w:body>
            </w:document>
            """.strip(),
        )
    ...
    assert (workdir / "lesson.md").read_text(encoding="utf-8") == "Hello HerPeakGem"


def test_mineru_signature_distinguishes_local_and_cloud() -> None:
    parser = factory.get_parser("mineru")
    from herpeakgem.services.parsing.engines.mineru.config import MinerUConfig
...
def test_mineru_cloud_readiness_needs_token() -> None:
    from herpeakgem.services.parsing.engines.mineru.config import MinerUConfig
    from herpeakgem.services.parsing.engines.mineru.readiness import mineru_readiness
...
def test_mineru_local_model_download_gate(monkeypatch: pytest.MonkeyPatch) -> None:
    from herpeakgem.services.parsing.engines.mineru import backend
    from herpeakgem.services.parsing.engines.mineru import readiness as rd
    from herpeakgem.services.parsing.engines.mineru.config import MinerUConfig
