...
from herpeakgem.services.rag.index_versioning import resolve_storage_dir_for_read
from herpeakgem.services.rag.pipelines.lightrag import config as lr_config
from herpeakgem.services.rag.pipelines.lightrag import engine, storage
from herpeakgem.services.rag.pipelines.lightrag.pipeline import LightRagPipeline
...
def test_embedding_func_returns_numpy_array(monkeypatch) -> None:
    ...
    monkeypatch.setattr("herpeakgem.services.embedding.get_embedding_config", lambda: _Config())
    monkeypatch.setattr("herpeakgem.services.embedding.get_embedding_client", lambda: _Client())
...
def test_lightrag_llm_adapter_preserves_messages_and_drops_extra_kwargs(
    ...
    monkeypatch.setattr("herpeakgem.services.llm.get_llm_client", lambda: _Client())
...
def _stub_parse(monkeypatch, *, blocks=None, markdown: str = "# md") -> None:
    from herpeakgem.services.parsing.types import ParsedDocument
    ...
    monkeypatch.setattr("herpeakgem.services.parsing.get_parse_service", lambda: _Service())
