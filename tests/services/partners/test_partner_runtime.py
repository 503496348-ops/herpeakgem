...
from herpeakgem.partners.bus.events import InboundMessage
from herpeakgem.partners.bus.queue import MessageBus
from herpeakgem.services.partners.manager import PartnerConfig
...
class _FakeOrchestrator:
    ...
    async def handle(self, context):
        from herpeakgem.services.memory.paths import memory_root
...
def fake_orchestrator(monkeypatch):
    import herpeakgem.runtime.orchestrator as orch_mod
    from herpeakgem.services.model_selection import runtime as selection_runtime
...
def _runner(partners_root, config: PartnerConfig | None = None) -> PartnerRunner:
    from herpeakgem.partners.config.paths import get_partner_sessions_dir
...
class TestTurnExecution:
    ...
    async def test_llm_config_error_folds_into_graceful_reply(
        ...
        from herpeakgem.services.llm.exceptions import LLMConfigError
        from herpeakgem.services.model_selection import runtime as selection_runtime
...
class TestContextAssembly:
    ...
    async def test_context_carries_soul_tools_and_metadata(self, partners_root, fake_orchestrator):
        from herpeakgem.services.partners.workspace import write_soul
    ...
    async def test_default_tools_resolve_to_full_toggleable_set(
        ...
        from herpeakgem.agents._shared.tool_composition import default_optional_tools
...
class TestBuiltinToolsAndMemory:
    ...
    async def test_turn_runs_against_partner_memory(self, partners_root, fake_orchestrator):
        ...
        from herpeakgem.partners.config.paths import get_partner_workspace
...
class TestLiveTurn:
    def test_buffer_replays_for_late_subscriber(self):
        from herpeakgem.services.partners.manager import LiveTurn
    ...
    async def test_web_turn_runs_on_instance_and_survives_resubscribe(
        ...
        from herpeakgem.services.partners.manager import PartnerManager
...
class TestPartnerCommands:
    ...
    async def test_sessions_resume_delete_commands(self, partners_root, fake_orchestrator):
        from herpeakgem.services.partners.commands import PartnerCommandHandler
    ...
    async def test_telegram_bot_command_suffix_is_supported(self, partners_root, fake_orchestrator):
        ...
        reply = await runner.process_message(_msg("/new@HerPeakGemBot"))
