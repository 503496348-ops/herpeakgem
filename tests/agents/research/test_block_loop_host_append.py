...
from herpeakgem.agents.research.data_structures import DynamicTopicQueue, ToolTrace
from herpeakgem.agents.research.pipeline import (
    LABEL_APPEND,
    LABEL_FINISH,
    LABEL_THINK,
    ResearchedBlock,
...
from herpeakgem.core.agentic.tool_dispatch import DispatchOutcome
...
def _make_pipeline(monkeypatch: pytest.MonkeyPatch) -> ResearchPipeline:
    ...
    monkeypatch.setattr("herpeakgem.agents.research.pipeline.get_llm_config", lambda: _FakeLLM())
    monkeypatch.setattr(
        "herpeakgem.agents.research.pipeline.get_tool_registry", lambda: _FakeRegistry()
    )
...
def _make_pipeline_with_registry(
    ...
    *,
    ...
    monkeypatch.setattr("herpeakgem.agents.research.pipeline.get_llm_config", lambda: _FakeLLM())
    monkeypatch.setattr("herpeakgem.agents.research.pipeline.get_tool_registry", lambda: registry)
    monkeypatch.setattr("herpeakgem.agents.research.pipeline.user_has_memory", lambda: False)
    monkeypatch.setattr("herpeakgem.agents.research.pipeline.user_has_notebooks", lambda: False)
    ...
    monkeypatch.setattr(
        "herpeakgem.agents.research.pipeline.exec_capability_available", lambda: True
    )
