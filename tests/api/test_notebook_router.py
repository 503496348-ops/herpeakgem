...
notebook_router = importlib.import_module("herpeakgem.api.routers.question_notebook").router
sessions_router = importlib.import_module("herpeakgem.api.routers.sessions").router
...
def store(tmp_path: Path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr(
        "herpeakgem.api.routers.question_notebook.get_sqlite_session_store",
        lambda: instance,
    )
    monkeypatch.setattr(
        "herpeakgem.api.routers.sessions.get_sqlite_session_store",
        lambda: instance,
    )
    return instance
