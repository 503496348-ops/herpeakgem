...
def _cleanup_question_router_module():
    yield
    sys.modules.pop("herpeakgem.api.routers.question", None)
...
def _load_question_router_module(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_agents = _package("herpeakgem.agents")
    fake_agents_question = types.ModuleType("herpeakgem.agents.question")
    fake_agents_question.AgentCoordinator = object
    fake_agents.question = fake_agents_question
    monkeypatch.setitem(sys.modules, "herpeakgem.agents", fake_agents)
    monkeypatch.setitem(sys.modules, "herpeakgem.agents.question", fake_agents_question)

    fake_logging = _package("herpeakgem.logging")
    ...
    monkeypatch.setitem(sys.modules, "herpeakgem.logging", fake_logging)

    fake_config = types.ModuleType("herpeakgem.services.config")
    fake_config.PROJECT_ROOT = Path.cwd()
    fake_config.load_config_with_main = lambda *_args, **_kwargs: {}
    monkeypatch.setitem(sys.modules, "herpeakgem.services.config", fake_config)

    fake_llm_package = _package("herpeakgem.services.llm")
    fake_llm_config = types.ModuleType("herpeakgem.services.llm.config")
    fake_llm_config.get_llm_config = lambda: None
    fake_llm_package.config = fake_llm_config
    monkeypatch.setitem(sys.modules, "herpeakgem.services.llm", fake_llm_package)
    monkeypatch.setitem(sys.modules, "herpeakgem.services.llm.config", fake_llm_config)

    fake_settings_package = _package("herpeakgem.services.settings")
    fake_interface_settings = types.ModuleType("herpeakgem.services.settings.interface_settings")
    ...
    monkeypatch.setitem(sys.modules, "herpeakgem.services.settings", fake_settings_package)
    monkeypatch.setitem(
        sys.modules,
        "herpeakgem.services.settings.interface_settings",
        fake_interface_settings,
    )

    fake_tools = _package("herpeakgem.tools")
    fake_tools_question = types.ModuleType("herpeakgem.tools.question")
    ...
    monkeypatch.setitem(sys.modules, "herpeakgem.tools", fake_tools)
    monkeypatch.setitem(sys.modules, "herpeakgem.tools.question", fake_tools_question)

    return importlib.import_module("herpeakgem.api.routers.question")
