...
if FastAPI is not None and TestClient is not None:
    knowledge_router_module = importlib.import_module("herpeakgem.api.routers.knowledge")
...
def test_create_preserves_known_nondefault_provider(monkeypatch, tmp_path: Path) -> None:
    ...
    pageindex_config = importlib.import_module("herpeakgem.services.rag.pipelines.pageindex.config")
...
def test_upload_task_marks_provider_failures_as_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "herpeakgem.knowledge.add_documents.RAGService",
        _FailingRagService,
    )
...
def test_reindex_accepts_default_alias(monkeypatch, tmp_path: Path) -> None:
    ...
    embedding_signature = importlib.import_module("herpeakgem.services.rag.embedding_signature")
    index_versioning = importlib.import_module("herpeakgem.services.rag.index_versioning")
    monkeypatch.setattr(
...
def test_update_config_coerces_legacy_provider_to_llamaindex() -> None:
    ...
    config_module = importlib.import_module("herpeakgem.services.config")
    app = _build_app()
