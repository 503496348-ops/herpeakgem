...
from herpeakgem.core.stream import StreamEvent, StreamEventType
from herpeakgem.services.llm.utils import clean_thinking_tags
from herpeakgem.services.path_service import get_path_service
from herpeakgem.services.session.protocol import SessionStoreProtocol

if TYPE_CHECKING:
    from herpeakgem.services.llm.config import LLMConfig
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from herpeakgem.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        from herpeakgem.services.session import get_session_store
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from herpeakgem.runtime.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from herpeakgem.multi_user.model_access import apply_allowed_llm_selection
            ...
            from herpeakgem.multi_user.model_access import (
        ...
        if llm_selection:
            ...
            from herpeakgem.services.model_selection import (
                LLMSelection,
        ...
        if payload.get("tools") is None:
            try:
                from herpeakgem.api.routers.settings import get_enabled_optional_tools
        ...
        # capability's turn.
        from herpeakgem.multi_user.tool_access import allowed_optional_tools
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from herpeakgem.agents.notebook import NotebookAnalysisAgent
            from herpeakgem.book.context import build_book_context
            from herpeakgem.core.context import Attachment, UnifiedContext
            from herpeakgem.runtime.orchestrator import ChatOrchestrator
            from herpeakgem.services.memory import get_memory_store
            from herpeakgem.services.model_selection.runtime import (
                activate_llm_selection,
            )
            from herpeakgem.services.model_selection.runtime import (
                reset_llm_selection as reset_active_llm_selection,
            )
            from herpeakgem.services.notebook import get_notebook_manager
            from herpeakgem.services.session.context_builder import ContextBuilder
            from herpeakgem.services.skill import get_skill_service
            ...
            from herpeakgem.services.storage import get_attachment_store
            ...
            from herpeakgem.utils.document_extractor import extract_documents_from_records
            ...
            from herpeakgem.multi_user.context import get_current_user
            from herpeakgem.multi_user.paths import get_admin_path_service
            from herpeakgem.multi_user.skill_access import assigned_skill_ids
            from herpeakgem.services.persona import PersonaService, get_persona_service
            from herpeakgem.services.skill.service import SkillService, render_skills_manifest
            ...
            if is_chat_capability:
                from herpeakgem.services.session.source_inventory import (
                    build_inventory,
                    render_manifest,
                )
    ...
    async def _maybe_generate_session_title(
        ...
        *,
        ...
        title = ""
        try:
            from herpeakgem.services.llm import stream as llm_stream
