...
from herpeakgem.services.cron.service import CronJob
...
async def _maybe_send_desktop_notification(job: CronJob, text: str) -> None:
    ...
    HerPeakGem still persists/delivers through its normal chat or partner
    ...
    title = f"HerPeakGem: {job.name or 'Reminder'}"
...
async def _execute_partner_job(job: CronJob) -> tuple[str, str | None]:
    ...
    if not delivery_meta.get("_streamed"):
        from herpeakgem.partners.bus.events import OutboundMessage
...
async def _execute_chat_job(job: CronJob) -> tuple[str, str | None]:
    ...
    from herpeakgem.multi_user.models import CurrentUser
    from herpeakgem.multi_user.paths import local_admin_user, scope_for_user, user_context
    from herpeakgem.runtime.orchestrator import ChatOrchestrator
    from herpeakgem.services.session import get_sqlite_session_store
