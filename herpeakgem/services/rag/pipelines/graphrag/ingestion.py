...
(its optional ``markitdown`` reader is just a converter). So HerPeakGem owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse HerPeakGem's existing extraction primitives here so multimodal/parsing
stays a HerPeakGem-side concern (matching how the LlamaIndex pipeline already
