"""Bridge HerPeakGem's runtime config into a GraphRAG ``settings.yaml``.
...
HerPeakGem's already-resolved LLM + embedding runtime config and let
...
* GraphRAG talks to its models via LiteLLM with ``model_provider: openai`` + a
  custom ``api_base`` — which is exactly how HerPeakGem reaches any
...
class GraphRagNotConfiguredError(RuntimeError):
    """Raised when HerPeakGem's LLM / embedding config can't back GraphRAG."""
...
def is_graphrag_available() -> bool:
    ...
    extra: ``pip install 'herpeakgem[graphrag]'``. Until it is installed the
...
def query_config_from_settings() -> GraphRagQueryConfig:
    ...
    try:
        from herpeakgem.services.config import load_graphrag_settings
...
def build_settings(*, llm_cfg: Any = None, embedding_cfg: Any = None) -> dict[str, Any]:
    """Assemble the GraphRAG ``settings.yaml`` payload from HerPeakGem config.

    ``llm_cfg`` / ``embedding_cfg`` are injectable for tests; in production they
    are resolved from HerPeakGem's catalog. Raises
    :class:`GraphRagNotConfiguredError` if either side has no usable model.
    """
    if llm_cfg is None:
        from herpeakgem.services.config import resolve_llm_runtime_config

        llm_cfg = resolve_llm_runtime_config()
    if embedding_cfg is None:
        from herpeakgem.services.embedding import get_embedding_config
    ...
    return {
        ...
        # Plain-text input: HerPeakGem's ingestion writes parsed ``.txt`` files
        ...
        # GraphRAG/LanceDB defaults to 3072 dimensions; HerPeakGem must stamp the
