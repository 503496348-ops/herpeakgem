"""Bridge HerPeakGem's runtime config into LightRAG / RAG-Anything.

LightRAG (AtomCollide-智械工坊/LightRAG) is a text knowledge-graph RAG engine; its multimodal
story is RAG-Anything (AtomCollide-智械工坊/RAG-Anything), built on top of LightRAG. The
...
builds the three adapters LightRAG needs from HerPeakGem's already-resolved LLM /
...
Decoupling notes:
* ``llm_model_func`` / ``vision_model_func`` wrap HerPeakGem's unified model
  callables and DROP LightRAG's internal kwargs (``hashing_kv``,
  ``keyword_extraction``, …) so they never leak into ``factory.complete``.
* ``embedding_func`` reuses HerPeakGem's embedding client, wrapped in LightRAG's
...
class LightRagNotConfiguredError(RuntimeError):
    """Raised when HerPeakGem's LLM / embedding config can't back LightRAG."""


def is_lightrag_available() -> bool:
    """True when RAG-Anything (which bundles LightRAG) can be imported.

    Opt-in extra: ``pip install 'herpeakgem[rag-lightrag]'``. Until installed the
...
def query_kwargs_from_settings() -> dict:
    ...
    """
    try:
        from herpeakgem.services.config import load_lightrag_settings
...
def build_llm_model_func():
    """Wrap HerPeakGem's unified LLM callable for LightRAG.
...
def build_vision_model_func():
    """Wrap HerPeakGem's vision-capable callable for RAG-Anything's image step."""
    from herpeakgem.services.llm import get_llm_client
...
def build_embedding_func():
    """Wrap HerPeakGem's embedding client in LightRAG's ``EmbeddingFunc``."""
    from lightrag.utils import EmbeddingFunc

    from herpeakgem.services.embedding import get_embedding_client, get_embedding_config
