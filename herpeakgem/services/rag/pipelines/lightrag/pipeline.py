...
Documents are turned into a MinerU-style ``content_list`` by HerPeakGem's shared
parse layer (``herpeakgem/services/parsing``) — the same bridge the question
...
from herpeakgem.services.rag.kb_paths import resolve_kb_dir
...
class LightRagPipeline:
    ...
    def _ensure_available(self) -> None:
        if not lr_config.is_lightrag_available():
            raise lr_config.LightRagNotAvailableError(
                ...
                "`pip install 'herpeakgem[rag-lightrag]'` to use LightRAG knowledge bases."
            )
    ...
    async def _ingest(self, rag: Any, file_paths: List[str]) -> int:
        ...
        """
        from herpeakgem.services.parsing import ParserError, get_parse_service
