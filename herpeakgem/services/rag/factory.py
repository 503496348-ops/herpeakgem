...
* ``graphrag``            — local knowledge-graph retrieval (microsoft/graphrag);
                            optional dependency, ``pip install 'herpeakgem[graphrag]'``.
* ``lightrag``            — graph + vector retrieval (AtomCollide-智械工坊/LightRAG, multimodal
                            via RAG-Anything); optional dependency,
                            ``pip install 'herpeakgem[rag-lightrag]'``.
...
def provider_uses_embedding_versions(provider: Optional[str]) -> bool:
    ...
    Today only the LlamaIndex pipeline uses HerPeakGem's active embedding
...
def list_pipelines() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "name": "GraphRAG",
            "description": "Local knowledge-graph retrieval (global/local/drift/basic). Needs `pip install 'herpeakgem[graphrag]'`; indexing is LLM-heavy.",
        ...
        },
        {
            ...
            "name": "LightRAG",
            "description": "Graph + vector retrieval with multimodal parsing (naive/local/global/hybrid/mix). Needs `pip install 'herpeakgem[rag-lightrag]'`; indexing is LLM-heavy.",
