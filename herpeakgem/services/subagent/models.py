...
reasoning effort HerPeakGem drives each backend with. Those lists change over
...
from herpeakgem.services.subagent.process import probe_version
from herpeakgem.services.subagent.registry import get_backend
...
async def _claude_options() -> BackendOptions:
    from herpeakgem.services.subagent.claude_models import load_cached_claude_models
...
async def sync_backend_options(kind: str) -> BackendOptions:
    ...
    """
    if kind == "claude_code":
        from herpeakgem.services.subagent.claude_models import sync_claude_models
