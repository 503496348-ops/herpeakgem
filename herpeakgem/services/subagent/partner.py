...
*partner session key*. The first consult of a HerPeakGem chat session has none,
so we mint a fresh ``dt-…`` key and return it; the cross-turn registry
(:mod:`herpeakgem.services.subagent.sessions`) remembers it against
(chat session, connection), so every later consult in the same HerPeakGem chat —
within one turn or across turns — resumes the SAME partner session. The partner
page then sees one complete history session per HerPeakGem chat, titled from the
...
if TYPE_CHECKING:  # avoid importing the core/partner packages at module load
    from herpeakgem.core.stream import StreamEvent
...
class PartnerBackend(SubagentBackend):
    ...
    async def consult(
        ...
        *,
        ...
        # a HerPeakGem chat → mint a stable, colon-free key; the registry threads
