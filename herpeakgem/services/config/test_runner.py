...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from herpeakgem.services.llm import clear_llm_config_cache, get_token_limit_kwargs
    ...
    async def _test_embedding(
        ...
        from herpeakgem.services.embedding.client import EmbeddingClient
        from herpeakgem.services.embedding.config import EmbeddingConfig
        ...
        probe_texts = [
            "HerPeakGem embedding smoke test",
            "HerPeakGem retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from herpeakgem.services.search import web_search
        ...
        run.emit("info", "Running search query: HerPeakGem configuration health check")
        result = web_search("HerPeakGem configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from herpeakgem.services.config.provider_runtime import resolve_tts_runtime_config
        from herpeakgem.services.voice import synthesize_speech
        ...
        sample = "HerPeakGem voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from herpeakgem.services.config.provider_runtime import resolve_stt_runtime_config
        from herpeakgem.services.voice import transcribe_audio
    ...
    async def _test_imagegen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from herpeakgem.services.config.provider_runtime import resolve_imagegen_runtime_config
        from herpeakgem.services.imagegen import generate_image
    ...
    async def _test_videogen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from herpeakgem.services.config.provider_runtime import resolve_videogen_runtime_config
        from herpeakgem.services.videogen import probe_video
