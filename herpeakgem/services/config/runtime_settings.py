...
# Document parsing settings. The parse layer (herpeakgem/services/parsing)
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with HerPeakGem's.
    "local_cli_path": "",
...
# HerPeakGem's VLM to describe images.
...
# HerPeakGem's legacy text extractors for PDF / Office / text-like files.
...
# LightRAG retrieval knobs (AtomCollide-智械工坊/LightRAG via RAG-Anything). ``top_k`` is the
