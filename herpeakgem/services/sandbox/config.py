...
:mod:`herpeakgem.tools.exec_tool`). Per-user quotas live in
:mod:`herpeakgem.services.sandbox.quota`.
"""
...
from herpeakgem.services.sandbox.backends import (
    BwrapBackend,
    RestrictedSubprocessBackend,
    RunnerSidecarBackend,
    SandboxBackend,
)
from herpeakgem.services.sandbox.spec import ResourceLimits
...
def build_backend(settings: SandboxSettings) -> SandboxBackend | None:
    ...
    backend's ``health()`` — see :mod:`herpeakgem.services.sandbox.service`.
    """
    import sys
