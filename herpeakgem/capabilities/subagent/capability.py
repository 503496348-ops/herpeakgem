...
(resolved by :mod:`herpeakgem.capabilities.subagent.binding`). As a
...
from herpeakgem.capabilities.protocol import KnowledgeCapability, PromptBlock
from herpeakgem.capabilities.subagent.binding import connection_for_turn
from herpeakgem.capabilities.subagent.tools import SUBAGENT_TOOL_NAMES
...
class SubagentCapability(KnowledgeCapability):
    ...
    def augment_kwargs(
        ...
        if conn is None:
            return kwargs
        from herpeakgem.services.subagent import load_subagent_settings
        from herpeakgem.services.subagent.sessions import get_session, session_key
        ...
        # session the user/HerPeakGem built up earlier (and the sidebar shares).
...
_DEFAULT_CONSULT_INSTRUCTION = (
    "You are being consulted programmatically by HerPeakGem on the user's behalf, "
...
def _resolve_budget(context: UnifiedContext) -> int:
    ...
    """
    from herpeakgem.services.subagent import load_subagent_settings
    from herpeakgem.services.subagent.config import CONSULT_BUDGET_MAX, CONSULT_BUDGET_MIN
...
def _system_text(language: str, name: str, budget: int, kind: str = "") -> str:
    from herpeakgem.services.subagent import PARTNER_BACKEND_KIND
