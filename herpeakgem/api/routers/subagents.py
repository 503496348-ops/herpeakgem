...
from herpeakgem.api.routers.auth import require_admin, require_auth
from herpeakgem.knowledge.kb_types import SUBAGENT_KB_TYPE
from herpeakgem.multi_user.knowledge_access import current_kb_manager
from herpeakgem.services.rag.linked_kb import assert_path_allowed
from herpeakgem.services.subagent import (
...
async def backend_options():
    ...
    from herpeakgem.services.subagent.models import list_backend_options
...
async def sync_backend(kind: str):
    ...
    """
    from herpeakgem.services.subagent import get_backend
    from herpeakgem.services.subagent.models import sync_backend_options
...
async def create_connection(payload: ConnectSubagentRequest, auth=Depends(require_auth)):
    ...
    consult within one HerPeakGem chat lands in that one partner session.
    """
...
async def delete_connection(name: str):
    ...
    from herpeakgem.services.subagent.sessions import forget_connection
...
async def message_connection(name: str, payload: SubagentMessageRequest):
    ...
    live session HerPeakGem consults (shared via the cross-turn registry, keyed by
    ...
    from herpeakgem.services.subagent import get_backend
    from herpeakgem.services.subagent.sessions import get_session, remember_session, session_key
