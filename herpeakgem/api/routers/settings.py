...
from herpeakgem.multi_user.model_access import allowed_llm_options
from herpeakgem.services.config import (
    get_config_test_runner,
    get_model_catalog_service,
    get_runtime_settings_service,
)
from herpeakgem.services.config.origins import normalize_origins
from herpeakgem.services.embedding.client import reset_embedding_client
from herpeakgem.services.llm.client import reset_llm_client
from herpeakgem.services.llm.config import clear_llm_config_cache
from herpeakgem.services.model_selection import list_llm_options
from herpeakgem.services.path_service import get_path_service
from herpeakgem.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
def _provider_choices() -> dict[str, list[dict[str, str]]]:
    ...
    from herpeakgem.services.config.provider_runtime import (
        EMBEDDING_PROVIDERS,
        IMAGEGEN_PROVIDERS,
        STT_PROVIDERS,
        TTS_PROVIDERS,
    ...
    from herpeakgem.services.provider_registry import PROVIDERS
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from herpeakgem.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from herpeakgem.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from herpeakgem.services.parsing.engines.mineru.backend import local_cli_probe
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from herpeakgem.services.parsing.engines.factory import get_parser, is_engine_available
...
async def start_mineru_models_download(payload: MinerUModelDownloadPayload):
    ...
    """
    _require_settings_admin()
    from herpeakgem.services.parsing.engines.mineru.models import (
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from herpeakgem.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from herpeakgem.services.parsing.engines.mineru.cloud import verify_credentials
    from herpeakgem.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from herpeakgem.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    """
    _require_settings_admin()
    from herpeakgem.services.llm.factory import fetch_models as fetch_llm_models
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. HerPeakGem will restart shortly.",
        "launch_at": launch_at,
