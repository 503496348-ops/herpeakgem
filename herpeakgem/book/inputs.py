...
Reuses:
- ``herpeakgem.services.session.get_sqlite_session_store`` for chat history
- ``herpeakgem.services.notebook.notebook_manager`` for notebook records
- ``herpeakgem.agents.notebook.NotebookAnalysisAgent`` for context distillation
"""
...
async def _resolve_notebook_context(
    ...
    *,
    ...
    if not notebook_refs:
        return "", 0
    try:
        from herpeakgem.services.notebook import notebook_manager
