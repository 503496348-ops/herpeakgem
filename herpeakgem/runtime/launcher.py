"""Local Web launcher for the installed HerPeakGem app."""
...
from herpeakgem.runtime.home import DEEPTUTOR_HOME_ENV, PACKAGE_ROOT, get_runtime_home
...
def _packaged_web_dir() -> Path | None:
    try:
        import herpeakgem_web
    except ImportError:
        return None
    path = Path(herpeakgem_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".herpeakgem-web-runtime.json"
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged HerPeakGem Web app.")
    ...
    raise SystemExit(
        "HerPeakGem Web assets are not installed. Install the full app with `pip install -U herpeakgem`, "
