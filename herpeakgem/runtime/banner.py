"""Branded banner + localized labels for ``herpeakgem start`` / ``herpeakgem init``.
...
from herpeakgem.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `herpeakgem start` to launch HerPeakGem.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `herpeakgem start` again."
        ),
        ...
        "start.port_in_use": (
            "HerPeakGem cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `herpeakgem start` 启动 HerPeakGem。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `herpeakgem start`。"
        ),
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]HerPeakGem[/]",
