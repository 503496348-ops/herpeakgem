...
from herpeakgem.runtime.request_contracts import (
...
class VisualizeCapability(BaseCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        from herpeakgem.agents.visualize.models import ReviewResult
        from herpeakgem.agents.visualize.pipeline import VisualizePipeline
        from herpeakgem.agents.visualize.utils import (
            build_fallback_html,
        ...
        *,
        ...
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'herpeakgem[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )

        from herpeakgem.agents.math_animator.pipeline import MathAnimatorPipeline
        from herpeakgem.agents.math_animator.request_config import MathAnimatorRequestConfig
        from herpeakgem.core.trace import build_trace_metadata, new_call_id
        from herpeakgem.services.llm.config import get_llm_config
