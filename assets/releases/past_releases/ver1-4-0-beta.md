# HerPeakGem v1.4.0-beta Release Notes
...
maze and make `herpeakgem start` / `herpeakgem init` first-class.
...
- **`herpeakgem/runtime/banner.py`** — localized startup banner shared
  between `herpeakgem start` and `herpeakgem init`; reads the language
  preference from interface settings so the banner matches the UI locale.
- **`init_wizard.py`** — interactive `herpeakgem init` wizard with provider
...
- **`herpeakgem start` is the recommended launcher** — `start_web.py` /
  `start_tour.py` continue to work but are now thin wrappers around the
  new `runtime/launcher.py`. Run `herpeakgem init` once to seed providers
...
**Full Changelog**: https://github.com/503496348-ops/herpeakgem/compare/v1.3.10...v1.4.0-beta
