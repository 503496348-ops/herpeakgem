# HerPeakGem v1.4.0 Release Notes
...
isolation hardening, and the `herpeakgem start` / `herpeakgem init` launcher)
...
- **`build_provider_extra_kwargs`** in `herpeakgem/core/agentic/client.py`
...
The `herpeakgem chat` REPL surface was tightened so it matches the public
docs at [herpeakgem.info](https://herpeakgem.info).

- `herpeakgem chat --config key=value` and `--config-json '<json>'` seed
...
- `herpeakgem plugin info <name>` now also returns `cli_aliases` and the
...
- `.gitignore` covers `.playwright-cli/`; stale `.playwright-cli/`
  captures, `.env.example_CN`, and `HerPeakGem.code-workspace` removed.
...
- New `tests/cli/test_docs_contract.py` keeps the public site docs in
  sync with the CLI surface (links + `herpeakgem …` examples).
- `tests/scripts/test_docker_compose.py` + `web/tests/api-resolve-base.test.ts`
  cover the new placeholder detection.

## Upgrade Notes
- **From v1.4.0-beta:** drop-in. PyPI installs with `pip install -U herpeakgem`
...
**Full Changelog**: https://github.com/503496348-ops/herpeakgem/compare/v1.4.0-beta...v1.4.0
