# HerPeakGem v1.4.6 Release Notes
...
- **Login** accepts a username (not just an email), surfaces bad-login errors
  ...
  flag.
- **CLI** gains `herpeakgem skill` commands — browser login plus publish/update
  for your own skills.
...
- Drop-in from v1.4.5: `pip install -U herpeakgem`; Docker users pull
  `ghcr.io/atomcollide/herpeakgem:latest`. No migrations — existing data loads as-is.
...
- New engines and services are opt-in: GraphRAG needs the extra
  (`pip install 'herpeakgem[graphrag]'`), document parsing needs
  `herpeakgem[parse]`, and voice / image / video need their model catalogs
  configured.

**Full Changelog**: https://github.com/503496348-ops/herpeakgem/compare/v1.4.5...v1.4.6
