...
CLI-only HerPeakGem distribution. It installs the `herpeakgem` command and the
...
FastAPI/Uvicorn server dependencies used by `herpeakgem start`.
...
the `herpeakgem` command at these source files.
