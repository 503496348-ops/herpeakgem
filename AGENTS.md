# HerPeakGem — Agent-Native Architecture

## Overview

HerPeakGem is an **agent-native** intelligent learning companion organized
...
`herpeakgem/capabilities/_shared.py` so every turn emits the same envelope
...
```bash
# Install
pip install herpeakgem      # Full app (CLI + Web/API + packaged Web assets)
pip install herpeakgem-cli  # CLI-only

# Run any capability
herpeakgem run chat "Explain Fourier transform"
herpeakgem run deep_solve "Solve x^2=4" -t rag --kb my-kb
herpeakgem run visualize "Animate sine wave" --config render_mode=manim_video
...
herpeakgem partner list

# Knowledge bases, memory, server
herpeakgem kb list
herpeakgem kb create my-kb --doc textbook.pdf
herpeakgem memory show
herpeakgem serve --port 8001       # API server only
herpeakgem start                   # backend + frontend together
```
...
| Path                                       | Purpose                              |
| ------------------------------------------ | ------------------------------------ |
| `herpeakgem/runtime/orchestrator.py`        | `ChatOrchestrator` — unified entry   |
| `herpeakgem/runtime/launcher.py`            | Backend + frontend lifecycle / port discovery |
| `herpeakgem/runtime/registry/`              | Tool + Capability registries         |
| `herpeakgem/runtime/bootstrap/builtin_capabilities.py` | Built-in capability class paths |
| `herpeakgem/services/config/runtime_settings.py` | JSON settings + process-env overrides |
| `herpeakgem/core/stream.py`, `stream_bus.py` | StreamEvent protocol + async fan-out |
| `herpeakgem/core/tool_protocol.py`          | `BaseTool` + `ToolDefinition`         |
| `herpeakgem/core/capability_protocol.py`    | `BaseCapability` + `CapabilityManifest` |
| `herpeakgem/core/context.py`                | `UnifiedContext` dataclass            |
| `herpeakgem/tools/builtin/__init__.py`      | All built-in tool wrappers           |
| `herpeakgem/capabilities/`                  | Built-in capability implementations  |
| `herpeakgem/app.py`                         | `HerPeakGemApp` — Python SDK facade    |
| `herpeakgem_cli/main.py`                    | Typer CLI entry point                |
| `herpeakgem/api/routers/unified_ws.py`      | Unified WebSocket endpoint           |
...
```
pip install herpeakgem      — Full app (CLI + Web/API + packaged Web assets)
pip install herpeakgem-cli  — CLI-only (LLM + RAG + providers + document parsing)
...
.[math-animator]  — Manim addon (powers `visualize` Manim renders + `herpeakgem run math_animator`)
