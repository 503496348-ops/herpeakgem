...
# lives at module scope so a single ``herpeakgem chat`` session shares one
# ring across turns; ``herpeakgem run`` doesn't read it (single-shot mode),
...
async def stream_turn_as_json(*, app: HerPeakGemApp, turn_id: str) -> None:
...
async def render_turn_stream(*, app: HerPeakGemApp, turn_id: str) -> str:
...
class TurnStreamRenderer:
    ...
    async def _on_wait_for_input(self, item: dict[str, Any]) -> None:
        ...
        from herpeakgem.core.stream_bus import get_bus
