...
``herpeakgem init`` walks the user through a four-step wizard (ports → LLM →
...
review panel) lives in :mod:`herpeakgem_cli.init_wizard`. This module is
...
from herpeakgem.runtime.home import DEEPTUTOR_HOME_ENV, get_runtime_home
...
def _reset_runtime_singletons() -> None:
    ...
    ``herpeakgem init`` may pass ``--home`` to target a different workspace; the
    singletons cache paths from the *previous* PathService and will silently
    write to the wrong place if not cleared.
    """
    try:
        from herpeakgem.services.path_service import PathService

        PathService.reset_instance()
    except Exception:
        pass
    try:
        from herpeakgem.services.config.runtime_settings import RuntimeSettingsService

        RuntimeSettingsService._instances.clear()
    except Exception:
        pass
    try:
        from herpeakgem.services.config.model_catalog import ModelCatalogService
...
def _embedding_step(
    ...
    from herpeakgem.services.config.embedding_endpoint import (
        EMBEDDING_PROVIDER_LABELS,
        normalize_embedding_endpoint_for_display,
    )
    from herpeakgem.services.config.provider_runtime import EMBEDDING_PROVIDERS
...
def run_init(*, cli_only: bool = False, home: str | Path | None = None) -> None:
    ...
    from herpeakgem.runtime.banner import labels_for, print_banner, resolve_language
    from herpeakgem.services.config import get_model_catalog_service, get_runtime_settings_service
