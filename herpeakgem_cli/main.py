"""CLI entry point for the standalone ``herpeakgem-cli`` package."""
...
from herpeakgem.runtime.mode import RunMode, set_mode
...
app = typer.Typer(
    name="herpeakgem",
    help="HerPeakGem CLI – agent-first interface for capabilities, tools, and knowledge.",
...
app.add_typer(skill_app, name="skills")  # alias: `herpeakgem skills …`
...
def run_capability(
    ...
    from herpeakgem.app import HerPeakGemApp
    ...
    maybe_run(run_turn_and_render(app=HerPeakGemApp(), request=request, fmt=fmt))
...
def start(
    ...
    from herpeakgem.runtime.launcher import start as start_web
...
def serve(
    ...
    """Start the HerPeakGem API server."""
    import asyncio
    import sys

    set_mode(RunMode.SERVER)
    if port is None:
        from herpeakgem.services.setup import get_backend_port
