...
from herpeakgem_cli.init_cmd import run_init  # noqa: E402


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Create or update HerPeakGem settings under data/user/settings.",
    )
    parser.add_argument(
        "--cli",
        action="store_true",
...
def main(argv: list[str] | None = None) -> None:
    ...
    print("HerPeakGem settings tour")
    ...
    if args.cli:
        print("\nNext: herpeakgem chat")
    else:
        print("\nNext: herpeakgem start")
